...
func TestMainTaskAndSubTasksAutoAndManual(t *testing.T) {
	...
	taskid, _ := scheduler.Tasks.AddTask(task_01)
	...
	taskidAuto, _ := scheduler.Tasks.AddTask(task_02)
	...
	taskidManual, _ := scheduler.Tasks.AddTask(task_03)

	for {
		state, _ := scheduler.Tasks.TaskState(taskid, "default")
		if state == "COMPLETED" {
			break
		}
	...
	for {
		state, _ := scheduler.Tasks.TaskState(taskidAuto, "default")
		if state == "COMPLETED" {
			break
		}
		time.Sleep(2 * time.Second)
	}
	scheduler.Tasks.AutoTask(taskidManual, "default")
	for {
		state, _ := scheduler.Tasks.TaskState(taskidManual, "default")
		if state == "COMPLETED" {
			break
		}
		time.Sleep(2 * time.Second)
...
func TestTasksWithSameDatetimeDifferentPriorities(t *testing.T) {
	...
	taskid_p9, _ := scheduler.Tasks.AddTask(task_p9)

	task_p2, _ := domain.NewTask("process", payload, queue, now, 2, domain.AUTO, "")
	taskid_p2, _ := scheduler.Tasks.AddTask(task_p2)

	task_p5, _ := domain.NewTask("process", payload, queue, now, 5, domain.AUTO, "")
	taskid_p5, _ := scheduler.Tasks.AddTask(task_p5)
...
func TestTasksWithDifferenteDatetime(t *testing.T) {
	...
	taskid_after_20_sec, _ := scheduler.Tasks.AddTask(task_p20)

	now = base_now.Add(5 * time.Second)
	task_p5, _ := domain.NewTask("process", payload, queue, now, 5, domain.AUTO, "")
	taskid_after_5_sec, _ := scheduler.Tasks.AddTask(task_p5)
